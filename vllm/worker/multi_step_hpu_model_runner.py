...
class HPUModelOutput:
    ...
    sampler_output_ready_event: torch.hpu.Event
    ...
    def pythonize(self, input_metadata: "HPUStatefulModelInput",
    ...
    def maybe_pythonize(self, input_metadata: "HPUStatefulModelInput",
                        copy_stream: torch.hpu.Stream,
                        pinned_sampled_token_buffer: torch.Tensor) -> None:
    ...
    def _pythonize_sampler_output(self, input_metadata: "HPUStatefulModelInput",
        ...
        with torch.hpu.stream(copy_stream):
        ...
        # own HPU stream, nonetheless _pythonize_sampler_output()
...
class HPUStatefulModelInput(BroadcastableModelInput):
    ...
    frozen_model_input: Optional[ModelInputForHPUWithSamplingMetadata] = None

    # list of model outputs for each step, may not be all pythonized
    cached_outputs: List[HPUModelOutput] = field(default_factory=list)
    ...
    ) -> "HPUStatefulModelInput":
        tensor_dict = _init_sampling_metadata_from_tensor_dict(tensor_dict)
        if attn_backend is not None:
            tensor_dict = _init_attn_metadata_from_tensor_dict(
                attn_backend, tensor_dict)
        tensor_dict = _init_frozen_model_input_from_tensor_dict(
            ModelInputForHPUWithSamplingMetadata, tensor_dict)

        return cls(**tensor_dict)

    def record_step_event(self, current_stream: torch.hpu.Stream):
    ...
    def wait_previous_step(self):
        # These hpu events are an explicit synchronization to ensure that
        ...
        # backend)
        self.step_hpu_events[(self.current_step + 1) & 1].wait()
...
class HPUMultiStepModelRunner(HPUModelRunnerBase[HPUStatefulModelInput]):
    ...
    def make_model_input_from_broadcasted_tensor_dict(
            self, tensor_dict: Dict[str, Any]) -> HPUStatefulModelInput:
        model_input = (HPUStatefulModelInput.from_broadcasted_tensor_dict(
            tensor_dict,
    ...
    def _final_process_outputs(self, model_input: HPUStatefulModelInput,
    ...
    def execute_model(
        ...
        if get_pp_group().is_last_rank and self.is_driver_worker:
            ...
            output_ready_event = torch.hpu.Event()
    ...
    def _advance_step(self, model_input: HPUStatefulModelInput,
                      out: SamplerOutput) -> HPUStatefulModelInput:
        frozen_model_input = model_input.frozen_model_input
        assert frozen_model_input is not None
