...
class cpu_ops:
    ...
    def silu_and_mul(out: torch.Tensor, x: torch.Tensor):
        ...
        def compile_silu_and_mul(out:torch.Tensor, x:torch.Tensor):
            d = x.shape[-1] // 2
            out.copy_(F.silu(x[..., :d]) * x[..., d:])
        compile_silu_and_mul(out, x)
    ...
    def gelu_and_mul(out: torch.Tensor, x: torch.Tensor):
        @torch.compile(dynamic=True, options={"fx_graph_cache": True})
        def compile_gelu_and_mul(out:torch.Tensor, x:torch.Tensor):
            d = x.shape[-1] // 2
            out.copy_(F.gelu(x[..., :d], approximate="none") * x[..., d:])
        compile_gelu_and_mul(out,x)
        # x1, x2 = xpu_ops.reshape_activation_tensor(x)
        # torch.ops.torch_ipex.gelu_mul(x1, x2, out, "none")

    def gelu_tanh_and_mul(out: torch.Tensor, x: torch.Tensor):
        @torch.compile(dynamic=True, options={"fx_graph_cache": True})
        def compile_gelu_tanh_and_mul( x:torch.Tensor) -> torch.Tensor:
            d = x.shape[-1] // 2
            return F.gelu(x[..., :d], approximate="tanh") * x[..., d:]
        out.copy_(compile_gelu_tanh_and_mul(x))
        # x1, x2 = xpu_ops.reshape_activation_tensor(x)
    ...
    def paged_attention_v1(
        ...
        cpu_key_cache = key_cache.squeeze(-1).movedim(2,3).movedim(1,2).contiguous()
        cpu_value_cache = value_cache.movedim(2,3).movedim(1,2).contiguous()
        ipex.llm.modules.PagedAttention.single_query_cached_kv_attention(
            out, query.contiguous(), cpu_key_cache,
            cpu_value_cache, head_mapping, scale, block_tables, context_lens,
            block_size, max_context_len, alibi_slopes)
        key_cache.copy_(cpu_key_cache.movedim(1,2).movedim(2,3).unsqueeze(-1).contiguous())
        value_cache.copy_(cpu_value_cache.movedim(1,2).movedim(2,3).contiguous())
...
class cpu_cache_ops:
    ...
    def reshape_and_cache(
        ...
        assert kv_cache_dtype == "auto"
        cpu_key_cache = key_cache.squeeze(-1).movedim(2,3).movedim(1,2).contiguous()
        print(cpu_key_cache.shape)
        cpu_value_cache = value_cache.movedim(2,3).movedim(1,2).contiguous()
        torch.ops.torch_ipex.reshape_and_cache(key, value, cpu_key_cache,
                                               cpu_value_cache, slot_mapping.int())
        key_cache.copy_(cpu_key_cache.movedim(1,2).movedim(2,3).unsqueeze(-1).contiguous())
        value_cache.copy_(cpu_value_cache.movedim(1,2).movedim(2,3).contiguous())
