...
class xpu_ops:
    ...
    def gelu_fast(out: torch.Tensor, x: torch.Tensor):
        torch.ops.torch_ipex.gelu_mul(x, x, out, "tanh")    
    ...
    def rotary_embedding(
        ...
        if is_neox:
            ...
            torch.ops.torch_ipex.apply_rotary_embedding_half_qk(query_rot, key_rot, sin, cos, query_rot, key_rot)
        else:
            cos = cos.repeat_interleave(2, dim = -1).unsqueeze(-2)
            sin = sin.repeat_interleave(2, dim = -1).unsqueeze(-2)
            torch.ops.torch_ipex.apply_rotary_embedding_two_qk(query_rot, key_rot, sin, cos, query_rot, key_rot)
