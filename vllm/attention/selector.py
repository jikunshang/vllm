...
def get_attn_backend(dtype: torch.dtype) -> AttentionBackend:
    if _can_use_flash_attn(dtype):
        ...
        logger.info("Using TorchSDPA backend.")
        from vllm.attention.backends.torch_sdpa import TorchSDPABackend  # noqa: F401
...
def _can_use_flash_attn(dtype: torch.dtype) -> bool:
    if is_hip():
        ...
        return False
    if is_xpu():
        logger.info("Cannot use FlashAttention backend for Intel GPUs.")
        return False
