...
#include <stdint.h>
#define U16_2_HALF(val) *(sycl::half*)(&val)
#define HALF_2_U16(val) *(uint16_t*)(&val)

#define U32_2_HALF2(val) *(sycl::half2*)(&val)
#define HALF2_2_U32(val) *(uint32_t*)(&val)
...
inline float half_to_float(uint16_t h) {
  return sycl::detail::half2Float(U16_2_HALF(h));
}
...
inline sycl::float2 half2_to_float2(uint32_t v) {
  ...
  tmp.u32 = v;
  return sycl::float2(half_to_float(tmp.u16[0]), half_to_float(tmp.u16[1]));
}

inline uint16_t float_to_half(float f) {
  auto val = sycl::detail::float2Half(f);
  return HALF_2_U16(val);
}
...
inline uint16_t add(uint16_t a, uint16_t b) {
  auto val = sycl::ext::intel::math::hadd(U16_2_HALF(a), U16_2_HALF(b));
  return HALF_2_U16(val);
}

inline uint32_t add(uint32_t a, uint32_t b) {
  auto val = sycl::ext::intel::math::hadd2(U32_2_HALF2(a), U32_2_HALF2(b));
  return HALF2_2_U32(val);
}
...
inline uint16_t mul(uint16_t a, uint16_t b) {
  auto val = sycl::ext::intel::math::hmul(U16_2_HALF(a), U16_2_HALF(b));
  return HALF_2_U16(val);
}

template <>
inline uint32_t mul(uint32_t a, uint32_t b) {
  auto val = sycl::ext::intel::math::hmul2(U32_2_HALF2(a), U32_2_HALF2(b));
  return HALF2_2_U32(val);
}
...
template <>
inline sycl::uint2 mul(sycl::uint2 a, sycl::uint2 b) {
  sycl::uint2 c;
...
template <>
inline sycl::uint2 mul(uint16_t a, sycl::uint2 b) {
  uint32_t s = h0_h0(a);
  sycl::uint2 c;
...
template <>
inline sycl::uint4 mul(sycl::uint4 a, sycl::uint4 b) {
  sycl::uint4 c;
  c.x() = mul<uint32_t, uint32_t, uint32_t>(a.x(), b.x());
  c.y() = mul<uint32_t, uint32_t, uint32_t>(a.y(), b.y());
...
template <>
inline sycl::uint4 mul(uint16_t a, sycl::uint4 b) {
  uint32_t s = h0_h0(a);
  sycl::uint4 c;
  c.x() = mul<uint32_t, uint32_t, uint32_t>(s, b.x());
  c.y() = mul<uint32_t, uint32_t, uint32_t>(s, b.y());
...
template <>
inline sycl::float2 mul(uint32_t a, uint32_t b) {
  sycl::float2 fa = half2_to_float2(a);
...
template <>
inline sycl::float2 mul(uint16_t a, uint32_t b) {
  return mul<sycl::float2, uint32_t, uint32_t>(h0_h0(a), b);
}

template <>
inline Float4_ mul(sycl::uint2 a, sycl::uint2 b) {
  Float4_ fc;
...
template <>
inline Float4_ mul(uint16_t a, sycl::uint2 b) {
  uint32_t s = h0_h0(a);
  Float4_ fc;
...
template <>
inline Float8_ mul(sycl::uint4 a, sycl::uint4 b) {
  Float8_ fc;
  fc.x = mul<sycl::float2, uint32_t, uint32_t>(a.x(), b.x());
  fc.y = mul<sycl::float2, uint32_t, uint32_t>(a.y(), b.y());
...
template <>
inline Float8_ mul(uint16_t a, sycl::uint4 b) {
  uint32_t s = h0_h0(a);
  Float8_ fc;
  fc.x = mul<sycl::float2, uint32_t, uint32_t>(s, b.x());
  fc.y = mul<sycl::float2, uint32_t, uint32_t>(s, b.y());
...
inline uint32_t fma(uint32_t a, uint32_t b, uint32_t c) {
  auto val = sycl::ext::intel::math::hfma2(
      U32_2_HALF2(a), U32_2_HALF2(b), U32_2_HALF2(c));
  return HALF2_2_U32(val);
}
...
template <>
inline float sum(sycl::uint2 v) {
  uint32_t c = add(v.x(), v.y());
  return sum(c);
}

template <>
inline float sum(sycl::uint4 v) {
  uint32_t c = add(v.x(), v.y());
  c = add(c, v.z());
  c = add(c, v.w());
  return sum(c);
...
inline void from_float(sycl::half& dst, float src) {
  dst = sycl::detail::float2Half(src);
}
