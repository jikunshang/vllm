...
void paged_attention_kernel(
  ...
  const sycl::nd_item<3> &item_ct1,
  uint8_t *dpct_local,
  sycl::local_accessor<vllm::Vec<scalar_t, VEC_SIZE>, 2> q_vecs,
  float *red_smem) {
...
#pragma unroll
  for (int i = thread_group_idx; i < NUM_VECS_PER_THREAD; i += NUM_THREAD_GROUPS) {
    ...
    q_vecs[thread_group_offset][i] = *reinterpret_cast<const Vec<scalar_t, VEC_SIZE>*>(q_ptr + vec_idx * VEC_SIZE);
  }
  /*
  ...
  */
  ...
  for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx; block_idx += NUM_WARPS) {
    ...
    for (int i = 0; i < NUM_TOKENS_PER_THREAD_GROUP; i++) {
      ...
      Q_vec q_vecs_new[NUM_VECS_PER_THREAD];
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        q_vecs_new[j] = *reinterpret_cast<const Q_vec*>(&q_vecs[thread_group_offset][j]);
      }

      // Compute dot product.
      // This includes a reduction across the threads in the same thread group.
      float qk = scale * Qk_dot<scalar_t, THREAD_GROUP_SIZE>::template dot<Q_vec, NUM_VECS_PER_THREAD>(q_vecs_new, k_vecs, item_ct1);
...
#pragma unroll
  ...
  for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx; block_idx += NUM_WARPS) {
    ...
    const int token_idx = block_idx * BLOCK_SIZE + physical_block_offset;
    L_vec logits_vec;
    vllm::from_float(logits_vec, *reinterpret_cast<Float_L_vec*>(logits + token_idx - start_token_idx));
...
void paged_attention_v1_kernel(
  ...
  const sycl::nd_item<3> &item_ct1,
  uint8_t *dpct_local,
  sycl::local_accessor<Vec<scalar_t, VEC_SIZE>, 2> q_vecs,
  float *red_smem) {
  paged_attention_kernel<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, VEC_SIZE>(
...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                   \
   queue.submit([&](sycl::handler &cgh) {                                    \
      ...
      sycl::local_accessor<Vec<T, VEC_SIZE>, 2> q_vecs_acc_ct1(                           \
      ...
      cgh.parallel_for(                                                        \
          ...
          [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {  \
             paged_attention_v1_kernel<T, HEAD_SIZE, BLOCK_SIZE,         \
